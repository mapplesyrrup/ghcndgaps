...
import { z } from "zod";
import { runMalfunctionAnalysis } from "@/lib/malfunction";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const result = await withTimeout(
      runMalfunctionAnalysis(variable, startDate, endDate, {
        latMin,
        latMax,
        lonMin,
        lonMax,
    ...
    if (err instanceof RequestTimeoutError) {
      console.error("Malfunction query timed out:", err);
      return NextResponse.json(
        {
          error:
    ...
    console.error(err);
    return NextResponse.json(
      { error: "Failed to compute malfunction analysis" },
      { status: 500 },
    );
  }
}
