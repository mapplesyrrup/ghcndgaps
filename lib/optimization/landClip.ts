...
export function landWithinBbox(bbox: BoundingBox): polygonClipping.MultiPolygon {
  const nearby = getLandPolygonsNear(bbox) as unknown as polygonClipping.MultiPolygon;
...
export function clipRingToLand(ring: Ring, land: polygonClipping.MultiPolygon): Ring[] {
  if (land.length === 0) return [];
  const clipped = polygonClipping.intersection([ring] as polygonClipping.Polygon, land);
...
function isPointOnLand(x: number, y: number, land: polygonClipping.MultiPolygon): boolean {
...
export function splitPathToLand(path: Ring, land: polygonClipping.MultiPolygon): Ring[] {
  if (land.length === 0) return [];
  const segments: Ring[] = [];
  let current: Ring = [];
